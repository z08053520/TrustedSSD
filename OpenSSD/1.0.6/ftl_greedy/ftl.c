...
// macro
//----------------------------------
#define INVALID             0xABABABAB
#define VC_MAX              0xCDCDCDCD
...
#define NUM_VCOUNT_SECT     ((VBLKS_PER_BANK * sizeof(UINT32) + BYTES_PER_SECTOR - 1) / BYTES_PER_SECTOR)
...
static UINT32 get_vcount(UINT32 const bank, UINT32 const vblock)
{
    ...
    vcount = read_dram_32(VCOUNT_ADDR + (((bank * VBLKS_PER_BANK) + vblock) * sizeof(UINT32)));
...
static void set_vcount(UINT32 const bank, UINT32 const vblock, UINT32 const vcount)
{
    ...
    write_dram_32(VCOUNT_ADDR + (((bank * VBLKS_PER_BANK) + vblock) * sizeof(UINT32)), vcount);
}
static UINT32 assign_new_write_vpn(UINT32 const bank)
{
    ASSERT(bank < NUM_BANKS);
...
static void format(void)
{
	...
	for (vblock = MISCBLK_VBN; vblock < VBLKS_PER_BANK; vblock++)
	{
		for (bank = 0; bank < NUM_BANKS; bank++)
		{
            ...
            write_dram_32(VCOUNT_ADDR + ((bank * VBLKS_PER_BANK) + vblock) * sizeof(UINT32),
                          vcount_val);
        }
    }
    //----------------------------------------
...
static void init_metadata_sram(void)
{
    ...
    for (bank = 0; bank < NUM_BANKS; bank++)
    {
        ...
        write_dram_32(VCOUNT_ADDR + ((bank * VBLKS_PER_BANK) + 0) * sizeof(UINT32), VC_MAX);
        write_dram_32(VCOUNT_ADDR + ((bank * VBLKS_PER_BANK) + 1) * sizeof(UINT32), VC_MAX);
        ...
        while (mapblk_lbn < MAPBLKS_PER_BANK)
        {
            ...
            if (is_bad_block(bank, vblock) == FALSE)
            {
                ...
                write_dram_32(VCOUNT_ADDR + ((bank * VBLKS_PER_BANK) + vblock) * sizeof(UINT32), VC_MAX);
                mapblk_lbn++;
            }
        }
        //----------------------------------------
...
void ftl_isr(void)
{
    ...
    for (bank = 0; bank < NUM_BANKS; bank++)
    {
        ...
        // dump current bank's BSP registers (last accepted FC)
        uart_print_32(bank);
