		      ...
		      UINT8  const num_sectors, UINT32 *sector_vals)
{
	...
	mem_set_dram(wr_buf, 111111, BYTES_PER_PAGE);	
		       ...
		       UINT8  const num_sectors, UINT32 *sector_vals)
{
	...
	UINT8	sp_begin = offset / SECTORS_PER_SUB_PAGE,
		sp_end	 = COUNT_BUCKETS((offset + num_sectors), SECTORS_PER_SUB_PAGE);
	UINT8	sp_i;
	for (sp_i = sp_begin; sp_i < sp_end; sp_i++) {
		...
		fu_read_sub_page(vsp, rd_buf, FU_SYNC);
	}

	UINT8	sect_i, sect_end = offset + num_sectors;
	for (sect_i = offset; sect_i < sect_end; sect_i++) {
		BOOL8	wrong = is_buff_wrong(rd_buf, sector_vals[sect_i], 
					      sect_i, 1);
...
{
	uart_printf("Test writing a 32KB page first, then overwrite it with a 5KB page...");
...
static void write_partial_page_then_whole_page()
{
	...
	offset = 0, num_sectors = SECTORS_PER_PAGE;
	set_vals(sector_vals, 6000, offset, num_sectors);
	write(lpn, offset, num_sectors, sector_vals);
...
static void write_partial_page_then_another_two_partial_pages()
{
	uart_printf("Test writing a 4KB page first, then overwrite it with a 12KB and 4KB page...");
	...
	offset = 4, num_sectors = 27;
	set_vals(sector_vals, 7000, offset, num_sectors);
	write(lpn, offset, num_sectors, sector_vals);

	offset = 32, num_sectors = 8;
	set_vals(sector_vals, 8000, offset, num_sectors);
	write(lpn, offset, num_sectors, sector_vals);

	ftl_write_task_force_flush();
	verify(lpn, 1, 39, sector_vals);
...
void ftl_test()
{
	...
	write_partial_page_then_another_two_partial_pages();
