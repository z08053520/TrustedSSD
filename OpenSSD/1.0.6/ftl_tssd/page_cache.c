...
/* For each cached sub page, we record **pmt_idx**, **timestamp** and **flag** */
...
void page_cache_init(void)
{
	...
	mem_set_sram(cached_pmt_idxes, NULL_PMT_IDX, 	num_bytes);
...
static void dump_state()
{
	if (show_debug_msg) {
		...
		UINT32 i, size = NUM_PC_SUB_PAGES;

		for (i = 0; i < size; i++) {
			if (i) uart_printf(", ");
			uart_printf("<%u>", cached_pmt_idxes[i]);
		}
		uart_print("]");
			 ...
			 UINT32 merged_pmt_idxes[SUB_PAGES_PER_PAGE])
{
	...
	for (sp_i = 0; sp_i < SUB_PAGES_PER_PAGE; sp_i++) {
		...
		merged_pmt_idxes[sp_i] = cached_pmt_idxes[page_idx];
...
{
	if (page_cache_has(pmt_idx)) {
		debug("\t\tthe page is in cache");
		UINT8 flag;
		page_cache_get_flag(pmt_idx, &flag);
	...
	page_cache_load_task_init(pc_load_task, pmt_idx);
