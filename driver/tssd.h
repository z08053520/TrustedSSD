...
/*
 * tssd.h -- definitions for the char module
 *
...
#    define PDEBUG(fmt, args...) printk( KERN_DEBUG "tssd: " fmt, ## args)
#  else
...
/*
 * The tssd device is removable: if it is left closed for more than
 * half a minute, it is removed. Thus use a usage count and a
 * kernel timer
 */

typedef struct Tssd_Dev {
   int size;
   int usage;
